...
[Choisis une des deux options et laisse uniquement celle-ci dans le prompt collé :]

**Option A — Checkpoint à chaque sprint (recommandé pour démarrer)**
Arrête-toi et attends ma validation à la fin de chaque sprint, comme décrit dans les garde-fous ci-dessus.

**Option B — Autopilote complet**
Ne t'arrête à la fin d'aucun sprint — enchaîne tous les sprints du backlog automatiquement, du Sprint 0 au Sprint 5, en respectant uniquement les garde-fous de blocage (contenu réel manquant, 3 échecs QA consécutifs). Donne-moi un résumé condensé après chaque sprint dans ta réponse, mais continue sans attendre de réponse de ma part.
...
| `nuxt-developer` | sonnet (opus en option pour US-040/US-041) | high | nuxt-remote, nuxt-ui-remote, supabase, context7 |
